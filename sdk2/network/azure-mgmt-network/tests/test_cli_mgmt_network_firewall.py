...
class MgmtNetworkTest(AzureMgmtTestCase):
    ...
    def test_network(self, resource_group):
        ...
        result = self.mgmt_client.azure_firewalls.update_tags(resource_group.name, AZURE_FIREWALL_NAME, BODY)
        result = result.result()

        # Delete Azure Firewall[delete]
        result = self.mgmt_client.azure_firewalls.delete(resource_group.name, AZURE_FIREWALL_NAME)
        result = result.result()
