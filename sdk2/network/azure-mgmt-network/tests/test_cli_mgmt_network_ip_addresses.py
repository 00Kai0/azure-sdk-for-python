...
class MgmtNetworkTest(AzureMgmtTestCase):
    ...
    def test_network(self, resource_group):
        ...
        #   }
        # }
        # result = self.mgmt_client.public_ipprefixes.create_or_update(resource_group.name, PUBLIC_IPPREFIX_NAME, BODY)
        ...
        result = self.mgmt_client.public_ipprefixes.get(resource_group.name, PUBLIC_IPPREFIX_NAME)
        ...
        result = self.mgmt_client.public_ipprefixes.list(resource_group.name)

        # List all public IP addresses[get]
        result = self.mgmt_client.public_ip_addresses.list_all()

        # List all public IP prefixes[get]
        result = self.mgmt_client.public_ipprefixes.list_all()
        ...
        result = self.mgmt_client.public_ipprefixes.update_tags(resource_group.name, PUBLIC_IPPREFIX_NAME, BODY)

        # Delete public IP address[delete]
        result = self.mgmt_client.public_ip_addresses.delete(resource_group.name, PUBLIC_IP_ADDRESS_NAME)
        result = result.result()

        # Delete public IP prefix[delete]
        result = self.mgmt_client.public_ipprefixes.delete(resource_group.name, PUBLIC_IPPREFIX_NAME)
        result = result.result()
